...
def create_historical_weather_reporting_table(in_table: Table, hot_day_celsius: float):
    return """
        SELECT
            time,
            city,
            temperature_2m_max AS day_max_temperature,
            SUM(
                CASE
...
def find_hottest_day_birthyear(in_table: pd.DataFrame, birthyear: int):
    ...
    try:
        ...
        # if my birthyear is not available, select the data from 2022
        df_birthyear = df[df["time"].str.startswith(str(birthyear))]
    ...
    # remane the columns
    output_df.colums = ["Date hottest day", "°C hottest day", "City", "lat", "long"]
...
def solution_transform_historical_weather():
    create_historical_weather_reporting_table(
        in_table=Table(name=c.IN_HISTORICAL_WEATHER_TABLE_NAME, conn_id=gv.CONN_ID_DUCKDB),
        hot_day_celsius=uv.HOT_DAY_CELSIUS,
        output_dataset=Table(name=c.OUT_HISTORICAL_WEATHER_REPORTING_TABLE_NAME, conn_id=gv.CONN_ID_DUCKDB),
    ),

    find_hottest_day_birthyear(
        in_table=Table(
            name=c.OUT_HISTORICAL_WEATHER_REPORTING_TABLE_NAME, conn_id=gv.CONN_ID_DUCKDB
        ),
        birthyear=uv.BIRTHYEAR,
        output_dataset=Table(name=c.OUT_HISTORICAL_WEATHER_REPORTING_TABLE_NAME, conn_id=gv.CONN_ID_DUCKDB),
    )
